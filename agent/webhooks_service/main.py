"""Standalone Tally form webhook -> outbound demo call service.

Deployed separately from the voice agent (which runs on Pipecat Cloud). When a lead
submits the Tally form on the website, Tally POSTs the submission to /webhooks/tally.
We pull out the lead's name and phone number, then place an outbound Twilio call to
them. Twilio requests TwiML from /twiml/outbound, which connects the call's media
stream to the Pipecat Cloud agent so Ava can greet the lead.
"""
...
async def twiml_outbound(name: str = "there"):
    """Return TwiML that connects an outbound call's media stream to the Pipecat Cloud agent."""
    agent_name = os.environ["PIPECAT_CLOUD_AGENT_NAME"]
    organization_name = os.environ["PIPECAT_CLOUD_ORGANIZATION_NAME"]
